...
int main(int argc, char **argv)
{
    ...
    for (int i = 0; i < tamanhoChunck; i++)
    {
        for (int j = 0; j < quantidadeDeBlocosPorFrame; j++)
        {
            correspondenciasDoFrame[count] = manipula(blocosDoFrame[i],count);

            // printf("resultado[%d] = (%d,%d)=>(%d,%d)\n", correspondenciasDoFrame[i].indiceFrame, correspondenciasDoFrame[i].xAtual, correspondenciasDoFrame[i].yAtual, correspondenciasDoFrame[i].xReferencia, correspondenciasDoFrame[i].yReferencia);
            count++;
        }
        
    }

    // i = 0 => i+j = 0,1,2,3,4,5,6
    // i = 1 => i+j = 
